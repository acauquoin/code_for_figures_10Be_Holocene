...
for i in range(len(lat_data)):
    if station[i] == 'South Pole':
        beryllium_flux_modeled_value = cdo.zonmean(input='-sellonlatbox,-180,180,-90,-88 -selvar,tot_be10 '+ifile_totdep_10be, options='--reduce_dim -f nc', returnArray='tot_be10')
        beryllium_conc_modeled_value = cdo.zonmean(input='-sellonlatbox,-180,180,-90,-88 -selvar,tot_be10_conc '+ifile_totdep_10be_conc, options='--reduce_dim -f nc', returnArray='tot_be10_conc')
        accu_modeled_value = cdo.zonmean(input='-sellonlatbox,-180,180,-90,-88 -selvar,aprt '+ifile_precip, options='--reduce_dim -f nc', returnArray='aprt')
        accu_modeled_value = accu_modeled_value * 1000 / 10000 * (60*60*24*365)
        elev = cdo.zonmean(input='-sellonlatbox,-180,180,-90,-88 -selvar,OROMEA '+ifile_jan_surf, options='--reduce_dim -f nc', returnArray='OROMEA')
    else:
